...
#+BEGIN_SRC python :results output scalar raw replace
  ...
  class BuckDesignParams():
      ...
      V_I             = 100      #  (V)   Nominal DC input voltage
      ...
      def Delta_i_L(self):
          ...
          return supplyOutputs.I_O_max() * RATIO_Delta_i_L

    
      def Delta_v_C(self):

          return supplyOutputs.V_O_nominal() * RATIO_Delta_v_C

    
      def L(self):
          L = (self.diodeBridgeOutputs.V_O_nominal                             \
  ...
  def compute_buck_design_params():
      ...
      buckDesignParams = BuckDesignParams(DiodeBridgeParams(),
                                          SupplyOutputs(LEDFixtureParams()))

      print("Delta_i_L = {0} A  L = {1} H"
