...
<Properties>
  <View=27,11,1354,837,1.06863,0,0>
  <Grid=10,10,1>
  <DataSet=bridge_1.dat>
  <DataDisplay=bridge_1.dpl>
  <OpenDisplay=1>
  <Script=bridge_1.m>
  <RunScript=0>
  <showFrame=0>
...
<Components>
  ...
  <C C1 0 760 310 17 -26 0 1 "914u" 1 "" 0 "neutral" 0>
  <NutmegEq NutmegBridge 1 1020 490 -27 15 0 0 "tran" 1 "P_L_t=V(Pr1) * vpr4#branch" 1 "P_I_t=V(Pr5) * vpr2#branch" 1 "V_I_rms=sqrt(mean(V(Pr5)^2))" 1 "I_I_rms=sqrt(mean(vpr2#branch^2))" 1 "V_O_rms=sqrt(mean(V(Pr1)^2))" 1 "I_O_rms=sqrt(mean(vpr4#branch^2))" 1 "V_O_ripple=vecmax(V(Pr1)) - vecmin(V(Pr1))" 1 "PF=mean(P_I_t)/(V_I_rms * I_I_rms)" 1>
  <.TR TR1 1 760 630 0 65 0 0 "lin" 1 "20 ms" 1 "60 ms" 1 "40001" 1 "Trapezoidal" 0 "2" 0 "1 ns" 0 "1e-16" 0 "150" 0 "0.001" 0 "1 pA" 0 "1 uV" 0 "26.85" 0 "1e-3" 0 "1e-6" 0 "1" 0 "CroutLU" 0 "no" 0 "yes" 0 "0" 0>
</Components>
<Wires>
